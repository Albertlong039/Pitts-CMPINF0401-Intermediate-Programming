   ...
   */
   ...
   public static void strReverse(String str)
   {
      ...
      else
      {
         ...
         strReverse(str.substring(0, str.length()-1)); // calls to itself and supplies the string from first character to next to last char "ie chops off last character here"
      }
   }
   static void printArray( int[] array, int start )
    {
        ...
        else
        {
            ...
            printArray(array,start+1); // we send in the same array again, but the start position is where it was initially advanced by one. What happens if it is the same?
